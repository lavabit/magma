...
/**
 * @brief   Clamp function for uint_t types.  Bound the return value of x
 *          between the min and max parameter vales.  
 * @author  Kent
 * @param   x     input number to be clamped
 * @param   min   minimum value for clamp
 * @param   max   maximum value for clamp
...
stacie_rounds_calculate (stringer_t *password, uint_t bonus)
{
	...
	// clamp hash_num to the range defined by MIN_ and MAX_
