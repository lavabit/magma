...
/**
 * @file /magma/src/objects/new_meta/serials.c
 *
 * @brief Functions for retrieving and incrementing new_meta object serial numbers.
 *
 * $Author$
 * $Date$
 * $Revision$
...
/**
 * @brief	Set an object serial number for a given new_meta user structure.
 *
 * @param	user	the new_meta user object to be adjusted.
...
/**
 * @brief	Get an object serial number for a given new_meta object.
 *
 * @param	user	the new_meta user structure to be examined.
 ...
 * @param	user	the new_meta user object to whom the object belongs.
...
bool_t new_meta_user_serial_check(new_meta_user_t *user, uint64_t object) {
	...
	if (user && new_meta_user_serial_get(user, object) == serial_get(object, user->usernum)) {
		new_meta_user_serial_set(user, object, serial_increment(object, user->usernum));
	}
