...
static bool_t is_locked(meta_user_t *user) {
	return user->lock_status != 0;
}

static chr_t * lock_error_message(meta_user_t *user) {
	chr_t *result;
...
void api_endpoint_auth(connection_t *con) {
	...
	login_result = credential_login(NULLER(username), NULLER(password), META_PROT_JSON, META_GET_NONE, &user);
	...
	if (is_locked(user)) {
		api_error(con, HTTP_ERROR_400, PORTAL_ENDPOINT_ERROR_AUTH, lock_error_message(user));
		goto cleanup;
	}

	con->http.session->state = SESSION_STATE_AUTHENTICATED;
	con->http.response.cookie = HTTP_COOKIE_SET;
	portal_endpoint_response(con, "{s:s, s:{s:s, s:s}, s:I}", "jsonrpc", "2.0", "result", "auth", "success", "session", st_char_get(con->http.session->warden.token), "id",
		con->http.portal.id);
