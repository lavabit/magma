...
static EC_KEY *
keys_enckey_from_binary(
    unsigned char const *bin_keys,
    size_t len);

static int keys_file_create(
    keys_type_t type,
    ...
    size_t len)
{
    ...
    RET_ERROR_CUST(
        KEYS_TYPE_ERROR,
        ERR_UNSPEC,
        "DIME number is not keys file type");
}
