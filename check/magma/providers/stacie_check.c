...
bool_t check_stacie_simple(void) {
	...
	if (!(combined_key = stacie_realm_key_derive(master_key, NULLER("mail"), shard)) ||
			!(vector_key = stacie_realm_vector_key(combined_key)) || st_cmp_cs_eq(vector_key, realm_vector_key) ||
			!(tag_key = stacie_realm_tag_key(combined_key)) || st_cmp_cs_eq(tag_key, realm_tag_key) ||
	...
	if (!(encrypted_buffer = stacie_realm_encrypt(0, vector_key, tag_key, cipher_key, PLACER("Attack at dawn!", 15))) ||
		!(decrypted_buffer = stacie_realm_decrypt(vector_key, tag_key, cipher_key, encrypted_buffer)) ||
...
*/
	...
	if (!(res1 = stacie_realm_vector_key(key)) ||
		!(res2 = stacie_realm_vector_key(key)) ||
		st_cmp_cs_eq(res1, res2)) {
		st_cleanup(res1, res2);
		return false;
	}
...
bool_t check_stacie_parameters(void) {
	...
	if ((res = stacie_realm_vector_key(NULL))) {
		st_free(res);
		return false;
	}

	if ((res = stacie_realm_vector_key(temp_st))) {
		st_free(res);
		return false;
	}
