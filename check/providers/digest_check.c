...
bool_t check_digest_simple(void) {
	...
	for (uint64_t i = 0; status() && i < (sizeof(digest_list) / sizeof(chr_t *)); i++) {
		...
		if (!(digest = digest_name(NULLER(digest_list[i]))) || !(hash = digest_hash(digest, PLACER(" ", 1), NULL))) {
			return false;
		}
...
bool_t check_digest_sthread(chr_t *name) {
	...
	for (uint64_t i = 0; status() && i < DIGEST_CHECK_ITERATIONS; i++) {
		...
		else if (!(digest = digest_name(NULLER(name))) || !(hash = digest_hash(digest, PLACER(" ", 1), NULL))) {
			return false;
		}
