 ...
 * @Author Ivan, refactored by Kent
 * $Date$
 * $Revision$
 *
 */
...
stacie_rounds_calculate (stringer_t *password, uint_t bonus) {
	...
	/*
     * Do you realize that between this call to st_empty() and the following 
	 * call to utf_length_get() calls st_empty 3 times?  Sloppy.
	...
	hash_num += bonus;
    hash_num = uint_clamp (hash_num, MIN_HASH_NUM, MAX_HASH_NUM);
	return hash_num;
