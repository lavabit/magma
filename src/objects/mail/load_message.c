...
stringer_t * mail_load_header(meta_message_t *meta, new_meta_user_t *user) {
	...
	if (!(uncompressed = cache_get(PLACER(key, keylen)))) {
		...
		if (meta->status & MAIL_STATUS_ENCRYPTED) {
			...
			if (!(unencrypted = ecies_decrypt(user->keys.private, ECIES_PRIVATE_BINARY, (cryptex_t *) raw, &dec_len))) {
...
mail_message_t * mail_load_message(meta_message_t *meta, new_meta_user_t *user, server_t *server, bool_t parse) {
	...
	if (!(raw = cache_get(PLACER(key, keylen)))) {
		...
		if (meta->status & MAIL_STATUS_ENCRYPTED) {
			...
			if (!(unencrypted = ecies_decrypt(user->keys.private, ECIES_PRIVATE_BINARY, (cryptex_t *)st_data_get(raw), &plain_len))) {
